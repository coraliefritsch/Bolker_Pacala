...
pop=CPo.population(parametres)           #pop est un objet contenant la population initiale et sa taille
...
############ ITERATIONS ############
#Les évènements ont lieu tant que le temps maximal n'est pas atteint, que la population ne s'est pas éteinte
#et que la population n'a pas atteint la taille maximale qu'on lui a imposée
while temps<parametres.temps_max and pop.taille>0 and pop.taille<parametres.taille_maximale_de_la_population:
    #On calcule le taux de compétition  subit par chaque individu : Il correspond au nombre de voisins
    #de l'individu * taux de stress engendré par un voisin 
    #Le taux de saut global de la population est alors:
    #    taux_global=taux_fixe*N+somme(competition des individus)
    #On tire le temps du prochain évènement par une loi exponentielle de taux taux_global
    #On choisi un individu dans la population (avec proba proportionnelle au taux de l'individu)
    #On décide ensuite le saut de la population (mort ou naissance) par la fonction saut
    competition=parametres.taux_de_mort_par_competition*np.array([ind.nbvoisins for ind in pop.individus[:pop.taille]])
